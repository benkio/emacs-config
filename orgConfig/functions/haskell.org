...
#+begin_src emacs-lisp :tangle yes
  ...
  (defun hs-goto-next-warn-error (&optional errorMessage backwardSearch isRegexp)
    ...
    (setq searchRegexp (concat ".*\\.hs:.*" errorMessage))
    (condition-case
        nil
  ...
  (defun hindent-buffer (input)
    "Run hindent on the file in input, if not existing, it just run on the current buffer"
    (interactive "bBuffer to indent: ")
    (setq commandExist nil)
    (condition-case nil
        (progn
          (call-process "hindent")
          (setq commandExist t)
          )
      (error (message "Please install hindent"))
      )


    (when commandExist
      (with-current-buffer input
       (shell-command (format "hindent %s" (buffer-file-name))))
      )
    )
  ...
  (global-set-key (kbd "C-c c h i") 'hindent-buffer)
#+end_src
