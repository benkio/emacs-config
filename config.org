...
*** Enable
    ...
    10. Better Scrolling
    11. Winner Mode: restore windows, especially for ediff sessions
    12. Save on frame focus out: https://emacsredux.com/blog/2014/03/22/a-peek-at-emacs-24-dot-4-focus-hooks/
    13. hs-minor-mode when coding
    14. Dired hide details by default
    15. ~term-line-mode~ on ~ansi-term~ to enable normal ~C-x~, ~M-x~ and editing
    16. ~org-agenda~ task files
    17. ~org-capture~ default file and template
    18. ~abbrev-mode~ always on
    19. Save abbrev silently
    20. Automatically add final newline on file save and visiting
    21. When you delete files they got into the thrash instead of permanently deleted
    22. Overwrite on region
