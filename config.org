  ...
  (defun dev-open-doc (queryType &optional lib)
    ...
    (interactive (list
                  ...
                  (completing-read "Library: "
                                   '("all" "angular" "backbone" "cpp" "coffeescript" "crystal" "elixir" "javascript" "julia" "jquery" "knockout" "kubernetes" "less" "lodash" "love" "marionette" "markdown" "matplotlib" "modernizr" "moment" "openjdk" "nginx" "numpy" "pandas" "postgresql" "python" "rails" "ruby" "rust" "scala" "sass" "tensorflow" "typescript" "underscore.js") nil nil)
                  ))
    (require 'browse-url)
...
#+begin_src emacs-lisp
  ...
  (defun ts-open-doc (queryType)
    "Open the typescript doc in browser searching for the input queryType. Uses the general dev-open-doc"
      (interactive (list
                    (read-string (format "type (%s): " (thing-at-point 'word)))))
      (funcall 'dev-open-doc queryType "typescript")
    )
#+end_src
*** C#
