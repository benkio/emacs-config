...
#+begin_src emacs-lisp
  (defun benkio-set-font-height-on-frame-width (&optional frame inputRatio)
    "Set font based on the frame size to show approximately 40 lines per screen.
Works for both client and standalone Emacs, independent of screen size.
Uses 180 as a reference font height and scales proportionally based on frame height."
    (interactive (list
                  (selected-frame)
                  (read-number "Font height (0 for auto):" 0)
                  )
                 )
    (setq frame (or frame (selected-frame)))  ;; Ensure frame is not nil
    (let* (
           ;; Get the frame's total height in pixels
           (framePixelHeight (frame-pixel-height frame))
           ;; Reference values: 180 font height works well for approximately 1080px frame height
           ;; This gives us a scaling factor to maintain ~40 lines per screen
           (referenceFrameHeight 1080)  ;; Frame height where 180 font height works well
           (referenceFontHeight 180)    ;; Known good font height value
           ;; Calculate font height proportionally to maintain ~40 lines
           ;; Formula: height = (currentFrameHeight / referenceFrameHeight) * referenceFontHeight
           (calculatedHeight (if (or (equal inputRatio nil) (= 0 inputRatio))
                                 (round (* (/ (float framePixelHeight) referenceFrameHeight)
                                           referenceFontHeight))
                               inputRatio))
           ;; Ensure reasonable bounds (80-300 is a safe range for font heights)
           (height (max 80 (min 300 calculatedHeight)))
           )

      (message "Frame height: %dpx, Calculated font height: %d (target: ~40 lines per screen)"
...
#+begin_src emacs-lisp
  ...
  (advice-add 'select-window :after (lambda (&rest _) (save-some-buffers t)))
  ...
  (advice-add 'ansi-term :after (lambda (&rest _) (term-line-mode)))
...
15. Org-mode to export html source code using classes instead of inline colors
#+begin_src emacs-lisp
  (run-at-time nil (* 5 60) 'recentf-save-list)
  (setq recentf-max-saved-items 50)

  (setq load-prefer-newer t)
  ;; Increase GC threshold during initialization for faster startup
  ;; Will be managed by gcmh-mode after initialization
...
Enable eglot LSP Server with configurations for supported languages
...
#+begin_src emacs-lisp
  (use-package eglot
    ...
    ;; Optional: Configure eglot settings
...
#+begin_src emacs-lisp
  (use-package ellama
    ...
    :config
    ;; Configure Ollama provider with auto-detection
    (require 'llm-ollama)
    (defcustom ellama-preferred-model nil
      "Preferred Ollama model name. If nil, auto-detects first available model.
Set this to a specific model name like \"llama3.2:3b\" to use a fixed model."
      :type '(choice (string :tag "Model name")
                     (const :tag "Auto-detect" nil))
      :group 'ellama)
    (defun ellama-setup-provider ()
      "Setup ellama provider with auto-detection or preferred model."
      (let ((model (or ellama-preferred-model
                       (condition-case nil
        ...
        (if model
            (setopt ellama-provider
                    (make-llm-ollama
                     ...
                     :host "localhost"
                     :port 11434))
          (message "Warning: No Ollama models found. Please run 'ollama pull <model>' or set ellama-preferred-model"))))
    ;; Setup provider after ellama is loaded
    (ellama-setup-provider)
...
Optimize garbage collection to reduce pauses during editing
https://github.com/emacscollective/gcmh
...
Makes emacs and magit aware of the system ssh-agent. Especially useful
to avoid authentication problems and misalignment
...
#+begin_src emacs-lisp
  (defun kill-all-buffers ()
    ...
    (condition-case err
        (progn
          (mapc (lambda (buf) (kill-buffer buf)) (buffer-list))
...
#+begin_src emacs-lisp
  ...
  (defun goto-definition (type typeDefinitionRegexp files)
  ...
  eg. \\(.*class  type .*\\|.*trait  type .*\\|.*object  type .*\\|.*type  type .*\\) to go to a scala definition
     ...
     - files: the type of files to search in the project (eg. scala, hs...)
  "
    (rg-project (funcall typeDefinitionRegexp type) files)
    )

  (defun build-import (inputType existingImportRegexp typeDefinitionRegexp build-import-from-existing-import-or-source importInsertionFunc files)
    "Template function to import a specific type:
     ...
     - files: The files to search in
  "
    (defun waitForRg ()
      "Wait for rg to finish and the buffer to be there"
      (let ((max-wait 300)  ; 30 seconds timeout (300 * 0.1)
            (wait-count 0)
            (rg-buffer nil)
            (rg-process nil))
        (while (and (< wait-count max-wait)
                    (progn
                      (setq rg-buffer (get-buffer "*rg*"))
                      (if rg-buffer
                          (progn
                            (pop-to-buffer "*rg*")
                            (setq rg-process (get-buffer-process rg-buffer))
                            (goto-char (point-min))
                            (if (search-forward "rg finished" nil t)
                                nil  ; Found "rg finished", exit loop
                              (progn
                                (if rg-process
                                    (message "rg is still running (process: %s)" (process-status rg-process))
                                  (message "rg is still running (waiting for process to start)"))
                                (sleep-for 0.1)
                                (setq wait-count (1+ wait-count))
                                t)))  ; Continue loop
                        (progn
                          (message "Waiting for rg buffer to be created...")
                          (sleep-for 0.1)
                          (setq wait-count (1+ wait-count))
                          t)))))  ; Continue loop
    ...
    (setq startingWindow (selected-window))
    (setq oneWindow (one-window-p))
    (rg-project (funcall existingImportRegexp inputType) files)
    (waitForRg)

    (when (search-forward "no matches found" nil t)
      (progn
        (kill-this-buffer)
        (select-window startingWindow)
        (when oneWindow (delete-other-windows))
        (goto-definition inputType typeDefinitionRegexp files)
        (waitForRg)
        ))
    ...
    (kill-this-buffer) ;; File with the import or definition
    (kill-buffer "*rg*")
    (select-window startingWindow)
  ...
  (defun dev-open-doc (queryType &optional lib)
    ...
    (require 'browse-url)
    (kill-new (or (s-present? queryType) (thing-at-point 'word t)))
...
#+begin_src emacs-lisp
  (setq scalaDefinitionRegex (lambda (type) (concat "\(.*class " type ".*\|.*trait " type ".*\|.*object " type ".*\|.*type " type ".*\|.*enum " type ".*\|.*def " type ".*\)")))
  ...
  (defun scala-goto-definition (typeOrFun)
    "Using the higher order function and the lambda defined above, it search in the project for the definition of the input type or function"
    (interactive (list
                  (read-string (format "type or fun (%s): " (thing-at-point 'word))
                               nil nil (thing-at-point 'word))))
    (goto-definition typeOrFun scalaDefinitionRegex "scala")
    )
...
Functions used specifically for dealing with java code.
Done by AI. In could be suboptimal

#+begin_src emacs-lisp
  (setq javaDefinitionRegex (lambda (type) (concat "\(.*class " type ".*\|.*interface " type ".*\|.*enum " type ".*\)")))
  (defun java-event-file-navigation (&optional notSplitWindow)
    "Navigate to the file that has a problem. it can navigate using a
        different window."
    (funcall 'event-file-navigation "\\(^\\|\\[ERROR\\] \\)" ":" notSplitWindow))

  (defun java-build-import-from-existing-import-or-source (type startingBuffer)
    "Considering the cursor is at the beginning of the target import line
           or into the java source file containing the definition of the
          target file. This functions return the import to insert into the
          dependent java source file."
    (if (string= (current-word) "import")
        (copy-line-from-point-as-string) ;; copy import line
      (concat "import " (java-file-path-to-package (buffer-file-name)) "." type ";") ;; copy package and make it an import
      ))

  (defun java-file-path-to-package (path)
    "transform a path to a package"
    (let ((java-path (or (nth 1 (split-string path "src/main/java/"))
                         (nth 1 (split-string path "src/java/"))
                         (nth 1 (split-string path "java/"))
                         (nth 1 (split-string path "src/")))))
      (if java-path
          (string-join
           (butlast
            (s-split "/" java-path))
  ...
  (defun java-path-to-package ()
    "transform a path to a package, current buffer"
    (interactive)
    (setq package (java-file-path-to-package (buffer-file-name)))
    (insert (concat "package " package ";"))
    )

                                          ; keybinded functions ;;;;;;;;;;;;;;;;;
  (defun java-shell-clean-old-output ()
    "When called on a shell buffer this function goes back to the beginning of the last compilation and delete the rest (old compilation)"
    (interactive)
    (funcall 'shell-clean-old-output "\\(> Task.* FAILED\\|gradle build\\|gradle run\\|\\[INFO\\] Scanning for projects...\\)"))

  (defun java-goto-next-warn-error (&optional errorMessage backwardSearch isRegexp)
    "Search into a maven/gradle output for the first warning/error, starting from cursor position, and move to it"
    (interactive)
    (goto-next-warn-error 'java-event-file-navigation ".*\\.java.*" ":" ":" '(lambda () (progn
                                                                                          (other-window -1)
                                                                                          (left-char)) ) errorMessage backwardSearch isRegexp))

  (defun java-import-bounds ()
    "Return the import region bounds"
    (save-excursion
      (search-backward-regexp "\\(;\|import\\)")
      (if (string= (current-word) "import")
          (progn
            (setq startOfImport (point)
                  endOfImport (line-end-position))
            (list startOfImport endOfImport)
            )
        (progn
          (setq endOfImport (line-end-position))
          (search-backward-regexp "\\(;\|import\\)")
          (if (string= (current-word) "import")
              (list (point) endOfImport)
            (list (point) endOfImport))
  ...
  (defun java-remove-unused-import ()
    "Parse a shell output in search of the first unused import and remove it"
    (interactive)
    (funcall 'remove-unused-import
             "\\(.*unused.*import\\|.*is never used\\|.*cannot be resolved\\)"
             'java-goto-next-warn-error
             'java-import-bounds
             (lambda (targetType)
               (delete-region (point-min) (point-max))           ;; single import
               )
  ...
  (defun java-import-type-at-point (type)
    "Try to import into the current file the type at point"
    (interactive (list
                  (read-string (format "type (%s): " (thing-at-point 'word))
                               nil nil (thing-at-point 'word))))
    (funcall 'build-import
             ...
             (lambda (type) (concat "import.*" type ".*;$"))
             javaDefinitionRegex
             'java-build-import-from-existing-import-or-source
             (lambda (result) (save-excursion
                                (beginning-of-buffer)
                                (end-of-line)
                                (search-forward-regexp "^$")
  ...
  (defun java-extract-code-line-or-region (name &optional parameters from to)
    "Extract the code to field or method:
           Require:
             - Name of the field/method
             - Optional list of parameters (if empty it will be a field)

           if no code region is selected then it extracts the rest of the line from current position
           Return type not specified.
          "
    (interactive (list
                  (read-string "field/method name: " )
                  (progn
                    (setq
                     param (read-string "param name (RET to finish): ")
                     params nil
    ...
    (funcall 'extract-code-line-or-region-template "private " " = " ";" "public void " "(" ")" " { " "}" name parameters from to)
    )

  (defun java-goto-definition (typeOrFun)
    "Using the higher order function and the lambda defined above, it search in the project for the definition of the input type or function"
    (interactive (list
                  (read-string (format "type or fun (%s): " (thing-at-point 'word))
                               nil nil (thing-at-point 'word))))
    (goto-definition typeOrFun javaDefinitionRegex "java")
    )

  (defun java-open-doc (queryType lib)
    "Open the java doc in browser searching for the input queryType"
    (interactive (list
                  (read-string (format "type (%s): " (thing-at-point 'word))
                               nil nil (thing-at-point 'word))
                  (completing-read "Library: "
                                   '("Standard Library" "Spring Framework" "Apache Commons" "Guava" "JUnit" "Mockito" "Jackson" "SLF4J" "Log4j" "Hibernate" "JPA" "JUnit 5" "AssertJ" "Lombok") nil t)
                  ))
    (require 'browse-url)
    (kill-new (or (s-present? queryType) (thing-at-point 'word t)))
    (setq libUrlPrefix (cond
                        ((string= lib "Standard Library") "https://docs.oracle.com/javase/8/docs/api/?search=")
                        ((string= lib "Spring Framework") "https://docs.spring.io/spring-framework/docs/current/javadoc-api/?search=")
                        ((string= lib "Apache Commons") "https://commons.apache.org/proper/commons-lang/javadocs/api-release/?search=")
                        ((string= lib "Guava") "https://guava.dev/releases/31.1-jre/api/docs/?search=")
                        ((string= lib "JUnit") "https://junit.org/junit4/javadoc/latest/?search=")
                        ((string= lib "JUnit 5") "https://junit.org/junit5/docs/current/api/?search=")
                        ((string= lib "Mockito") "https://javadoc.io/doc/org.mockito/mockito-core/latest/?search=")
                        ((string= lib "Jackson") "https://fasterxml.github.io/jackson-databind/javadoc/2.13/?search=")
                        ((string= lib "SLF4J") "https://www.slf4j.org/api/?search=")
                        ((string= lib "Log4j") "https://logging.apache.org/log4j/2.x/log4j-api/apidocs/?search=")
                        ((string= lib "Hibernate") "https://docs.jboss.org/hibernate/orm/5.6/javadocs/?search=")
                        ((string= lib "JPA") "https://javaee.github.io/javaee-spec/javadocs/?search=")
                        ((string= lib "AssertJ") "https://assertj.github.io/doc/?search=")
                        ((string= lib "Lombok") "https://projectlombok.org/api/?search=")
                        (t                           "https://docs.oracle.com/javase/8/docs/api/?search=")
                        ))
    (browse-url (concat libUrlPrefix queryType))
    )

  (defun java-github-search-open-repo (queryType repo)
    "Search on github for the querytype in the given repo and open the selected file in a separate buffer"
    (interactive (list
                  (read-string (format "searchTerm (%s): " (thing-at-point 'word))
                               nil nil (thing-at-point 'word))
                  (completing-read "repository: "
                                   '("spring-projects/spring-framework" "apache/commons-lang" "google/guava" "junit-team/junit4" "junit-team/junit5" "mockito/mockito" "FasterXML/jackson" "projectlombok/lombok" "assertj/assertj-core") nil nil)
                  ))
    (github-search-open-repo queryType repo)
    )
#+end_src
...
#+begin_src emacs-lisp
  ...
  (defun hs-goto-definition (type)
    ...
    (interactive (list
                  (read-string (format "type (%s): " (thing-at-point 'word))
                               nil nil (thing-at-point 'word))))
    (goto-definition type haskellDefinitionRegex "haskell")
    )
...
Functions useful when dealing with PureScript.

#+begin_src emacs-lisp

  (setq purescriptDefinitionRegex (lambda (type) (concat "\(.*data " type ".*\|.*type " type ".*\|.*newtype " type ".*\|" type " ::.*\)")))

  (defun ps-shell-clean-old-output ()
    "When called on a shell buffer this function goes back to the beginning of the last compilation and delete the rest (old compilation)"
    (interactive)
    (funcall 'shell-clean-old-output "\\(Compiling\\|spago build\\|spago test\\|[1-9][0-9]? error[s]?\\|[1-9]+ warning[s]?\\|Build succeeded\\|psci> \\)"))

  (defun ps-event-file-navigation (&optional notSplitWindow)
    "Navigate to the file that has a problem. it can navigate using a
     different window."
    (beginning-of-line) ;; Often called from the end of the line of the file path targeted
    (funcall 'event-file-navigation "at " ":" notSplitWindow))

  (defun ps-goto-next-warn-error (&optional errorMessage backwardSearch isRegexp)
    "Search into a spago output for the first warning/error, starting from cursor position, and move to it"
    (interactive)
    (goto-next-warn-error 'ps-event-file-navigation ".*\\.purs:.*" ":" "[:-]" '(lambda () () ) errorMessage backwardSearch isRegexp))

  (defun ps-extract-code-line-or-region (name &optional parameters from to)
    "Extract the code to val or def:
      Require:
        - Name of the val/def
        - Optional list of parameters (if empty it will be a val)
  ...
  (defun ps-goto-next-unused-import (&optional errorMessage backwardSearch isRegexp)
    "Search into a spago output for the unused import, and move to it.
      Special case of ps-goto-next-warn-error since the output doesn't provide
      the correct column position"
    (interactive)
    (unless errorMessage (setq errorMessage ""))
    (unless isRegexp (setq errorMessage (regexp-quote errorMessage)))
    (setq searchRegexp (concat (getenv "HOME") ".*\\.purs.*" errorMessage))
    (condition-case
        nil
        (funcall (searchFunction backwardSearch) searchRegexp)
      (error (user-error "no match found for %s" errorMessage))
      )
    (ps-event-file-navigation t)
    (other-window -1)
  ...
  (defun ps-remove-unused-import ()
    "Parse a shell output (spago) in search of the first unused import and remove it"
    (interactive)
    (funcall 'remove-unused-import
             ".*Unused import.*$"
             'ps-goto-next-unused-import
             'ps-import-bounds
             (lambda (targetType)
               (if (search-forward "," nil t)
                   (progn ;; multi import
  ...
  (defun ps-build-import-from-existing-import-or-source (type startingBuffer)
    "Considering the cursor is at the beginning of the target import line
      or into the purescript source file containing the definition of the
     target file. This functions return the import to insert into the
     dependent purescript source file."
    (if (string= (current-word) "import")
        (copy-line-from-point-as-string) ;; copy import line
      (progn
        (beginning-of-buffer)
  ...
  (defun ps-import-type-at-point (type)
    "Try to import into the current file the type at point"
    (interactive (list
                  (read-string (format "type (%s): " (thing-at-point 'word))
                               nil nil (thing-at-point 'word))))
    (funcall 'build-import
             type
             (lambda (type) (concat "^import .*" type "[ ,]?.*\\)$"))
             purescriptDefinitionRegex
             'ps-build-import-from-existing-import-or-source
             (lambda (result) (save-excursion
                                (beginning-of-buffer)
                                (search-forward "where")
                                (next-line)
  ...
  (defun ps-string-to-multiline-string (&optional $from $to)
    "Escape the string selected as purescript multiline string"
    (interactive
     (if (use-region-p)
         (list (region-beginning) (region-end))
       (let ((bds (bounds-of-thing-at-point 'paragraph)) )
  ...
  (defun ps-goto-definition (type)
    "Using the higher order function and the lambda defined above, it search in the project for the definition of the input type"
    (interactive (list
                  (read-string (format "type (%s): " (thing-at-point 'word))
                               nil nil (thing-at-point 'word))))
    (goto-definition type purescriptDefinitionRegex "purs")
    )

  (defun ps-pursuit (pursuitSearch)
    "Open a Browser tab and search the term in Pursuit (PureScript documentation)"
    (interactive (list
                  (read-string (format "pursuit search (%s): " (thing-at-point 'word))
                               nil nil (thing-at-point 'word))))
    (require 'browse-url)
    (browse-url (concat "https://pursuit.purescript.org/search?q=" pursuitSearch))
    )
#+end_src
...
#+begin_src emacs-lisp
  (setq typescriptDefinitionRegex (lambda (type) (concat "\(.*class " type ".*\|.*interface " type ".*\|.*const " type ".*\|.*type " type ".*\)")))
  ...
  (defun ts-build-import-from-existing-import-or-source (type startingBuffer)
    ...
    (if (string= (current-word) "import")
      ...
      (let ((target-file (buffer-file-name))
            (source-dir (file-name-directory (buffer-file-name (get-buffer startingBuffer)))))
        (concat "import { " type " } from '" (s-chop-suffixes '(".ts" ".tsx" ".ts.html") (file-relative-name target-file source-dir))  "';")
        )
      )
    )
  ...
  (defun ts-goto-definition (type)
    ...
    (interactive (list
                  (read-string (format "type (%s): " (thing-at-point 'word))
                               nil nil (thing-at-point 'word))))
    (goto-definition type typescriptDefinitionRegex "typescript")
    )
  ...
  (defun ts-open-doc (queryType lib)
    ...
    (kill-new (or (s-present? queryType) (thing-at-point 'word t)))
    (setq libUrlPrefix (cond
                        ((string= lib "Effect-ts")   "https://effect-ts.github.io/effect")
...
#+begin_src emacs-lisp
  ...
  (defun csharp-goto-definition (type)
    ...
    (interactive (list
                  (read-string (format "type (%s): " (thing-at-point 'word))
                               nil nil (thing-at-point 'word))))
    (goto-definition type csharpDefinitionRegex "csharp")
    )
#+end_src
** Dired
  ...
  With prefix argument TIMES, run it that many times."
    (interactive
     (list (read-command "Command to replay: ")
           ...
           (read-number "Delay in seconds: " 0.5)))
    (setq counter 0
          internalTimes times
...
*** Java
#+begin_src emacs-lisp
  (global-set-key (kbd "C-c c j c") 'java-shell-clean-old-output)
  (global-set-key (kbd "C-c c j e") 'java-goto-next-warn-error)
  (global-set-key (kbd "C-c c j u") 'java-remove-unused-import)
  (global-set-key (kbd "C-c c j p") 'java-import-type-at-point)
  (global-set-key (kbd "C-c c j P") 'java-path-to-package)
  (global-set-key (kbd "C-c c j x") 'java-extract-code-line-or-region)
  (global-set-key (kbd "C-c c j g") 'java-goto-definition)
  (global-set-key (kbd "C-c c j G") 'java-github-search-open-repo)
  (global-set-key (kbd "C-c c j d") 'java-open-doc)
#+end_src
*** Haskell
#+begin_src emacs-lisp
...
*** PureScript
#+begin_src emacs-lisp
  (global-set-key (kbd "C-c c p c") 'ps-shell-clean-old-output)
  (global-set-key (kbd "C-c c p e") 'ps-goto-next-warn-error)
  (global-set-key (kbd "C-c c p u") 'ps-remove-unused-import)
  (global-set-key (kbd "C-c c p p") 'ps-import-type-at-point)
  (global-set-key (kbd "C-c c p x") 'ps-extract-code-line-or-region)
  (global-set-key (kbd "C-c c p s") 'ps-string-to-multiline-string)
  (global-set-key (kbd "C-c c p d") 'ps-goto-definition)
  (global-set-key (kbd "C-c c p h") 'ps-pursuit)
#+end_src
*** Typescript
#+begin_src emacs-lisp
