...
*** Frame Font Size
    ...
    #+BEGIN_SRC emacs-lisp
      (defun set-font-height-on-frame-width (&optional frame ratio)
        ...
        (interactive (list
                        (selected-frame)
                        (read-number "Ratio (130, less bigger, more smaller):" 130)
                      )
                     )
...
*** Set
    ...
    12. Basic ERC Config
    13. `emacs-startup-hook` to run functions that has to run at the end of the initialization
...
*** Hurl
    ...
    #+BEGIN_SRC emacs-lisp
      ...
      (use-package hurl-mode
        :init (slot/vc-install :fetcher "github" :repo "JasZhe/hurl-mode")
        :mode "\\.hurl\\'"
        )
    #+END_SRC
...
** Maestral
  ...
  #+begin_src emacs-lisp
    (defun get-or-create-dropbox-link (file)
      "Given an input file in Dropbox, this functions copy the Dropbox link related to it"
      (interactive "FDrobox file: ")
      ...
      (if (> (length dropboxLinks) 1) ; if we have 2 links and an empty string, the first is what we want
          (progn (message (format "Get Dropbox Link for %s from %d links: %s" file (length dropboxLinks) dropboxLinks))
                 (kill-new (car dropboxLinks)))
        (progn
          ...
          (kill-new (car dropboxLinksCreate))
          ))
      )
  #+end_src
