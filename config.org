...
** Highlight
   ...
   Sometimes you want to highlight stuff in a buffer.
...
** FFMPEG
   ...
   #+begin_src emacs-lisp
     ...
     sNew Name: ")
       ...
       (setq ffmpegCommand (concat "ffmpeg -ss " startTime " -i \"" origin "\" -t " (number-to-string totalSeconds) " -c:v copy -c:a copy \"" (concat (file-name-directory origin) newName) "\""))
...
*** Youtube-dl
    ...
    #+begin_src emacs-lisp
      ...
      (defun youtube-dl (youtubeUrl destinationPath outputFormat)
        ...
        (cond
         ((member outputFormat supportedAudioFormats) (setq youtubeDlCommand (format "youtube-dl -x --audio-format %s -o '%s%%(title)s-%%(id)s.%%(ext)s' %s" outputFormat destinationPath youtubeUrl)))
         ((member outputFormat supportedVideoFormats) (setq youtubeDlCommand (format "youtube-dl --recode-video %s -o '%s%%(title)s-%%(id)s.%%(ext)s' %s" outputFormat destinationPath youtubeUrl)))
         (t (error (message "Please insert a valid output format: %s" outputFormat)))
         )
        (when commandExist
          (progn
            (async-shell-command youtubeDlCommand)
            )
          )
        )
      ...
      (defun scala-open-doc (queryType lib)
        ...
        (interactive (list
                      ...
                      (completing-read "Library: "
                                       '("Standard Library" "Cats" "Cats-Effect" "Http4s 0.21" "fs2 core 3" "Circe" "Munit core 2.13" "Skunk core 2.13" "Monocle core 2.13" "Scalacheck") nil t)
                      ))
        (require 'browse-url)
