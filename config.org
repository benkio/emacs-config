...
#+begin_src emacs-lisp
  ...
  (defun java-event-file-navigation (&optional notSplitWindow)
    "Navigate to the file that has a problem. it can navigate using a
          different window. Handles Gradle/Maven error format: /path/to/file.java:line:"
    (let ((current-point (point)))
      (beginning-of-line)
      (when (search-forward-regexp "\\.java:" (line-end-position) t)
        (left-char) ; move back to just before the colon
        (setq filenamePathEnd (point))
        (search-backward-regexp "^\\| " nil t)
        (when (looking-at " ")
          (forward-char))
        (setq filePathStart (point))
        (setq filePath (expand-file-name (string-trim (buffer-substring filePathStart filenamePathEnd))))
        (goto-char current-point) ; restore cursor position for line/column parsing
        (when (and (not notSplitWindow) (= (count-windows) 1)) (split-window-right))
        (other-window 1)
        (message "find file: %s" filePath)
        (find-file (string-trim filePath)))))
  ...
  (defun java-shell-clean-old-output ()
    "When called on a shell buffer this function goes back to the beginning of the last compilation and delete the rest (old compilation)"
    (interactive)
    (funcall 'shell-clean-old-output "\\(\\[INFO\\] Building\\|\\[ERROR\\] BUILD FAILURE\\|BUILD SUCCESS\\|^> \\)"))
