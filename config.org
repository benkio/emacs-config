    ...
    #+begin_src emacs-lisp
      ...
      (defun github-search-open-repo (searchString repo)
        ...
        (setq searchQueryParameter (concat searchString "+in:file+repo:" repo)
              ...
              matchingFilesJson (gethash "items" searchJsonFull)
              selectedFile (completing-read "select target file: " (mapcar (lambda (x) (gethash "name" x)) matchingFilesJson))
              selectedElemJson (elt (remove-if-not (lambda (x) (string-equal (gethash "name" x) selectedFile)) matchingFilesJson) 0)
      ...
      (defun scala-github-search-open-repo (queryType repo)
        ...
        (interactive (list
                      ...
                      (completing-read "repository: "
                                       '("typelevel/cats" "typelevel/cats-effect" "http4s/http4s" "typelevel/fs2" "circe/circe" "scalameta/munit" "tpolecat/skunk" "optics-dev/Monocle") nil t)
                      ))
