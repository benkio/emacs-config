...
*** Higher Order & Cross Language Functions
    ...
    #+begin_src emacs-lisp
      ...
      (defun github-search-open-repo (searchString repo)
        ...
        (message "key %s" cacheKeySelectedFiles)
        (message "value %s" cacheSelectedFilesJson)
        (setq
        ...
        (if cacheRawContent
          ...
          (progn
            (setq selectedElemJson (elt (remove-if-not (lambda (x) (string-equal (gethash "name" x) selectedFile)) matchingFilesJson) 0)
