...
#+begin_src emacs-lisp
  ...
  ;; Only refresh package contents if they don't exist
  ;; This avoids slow startup on every launch
...
#+begin_src emacs-lisp
  ...
  (defun benkio-synchronize-theme ()
    "Synchronize theme based on time of day (light 6-15, dark otherwise)"
    (let ((hour (string-to-number
                 (substring (current-time-string) 11 13)))
          (new-theme (if (member hour (number-sequence 6 15))
                         #'benkio-light-theme
                       #'benkio-dark-theme)))
      (unless (eq new-theme current-theme)
